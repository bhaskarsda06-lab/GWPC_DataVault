-- Phase 9.7: Staging -> Hub Account count reconciliation
WITH stg AS (
    SELECT COUNT(DISTINCT publicid) cnt FROM {{ ref('stg_account') }}
    WHERE COALESCE(TRIM(publicid),'') <> ''
), hub AS (
    SELECT COUNT(DISTINCT source_system_unique_identifier) cnt FROM {{ ref('hub_account') }}
