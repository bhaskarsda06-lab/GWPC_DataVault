-- Phase 10.4: Production Post-Deployment Smoke Test
-- READ ONLY. No CREATE/INSERT/UPDATE/DELETE statements.
...
-- Critical-key smoke check
SELECT
    'critical_key_nulls' AS check_name,
    COUNT(*) AS failure_count
FROM (
    SELECT account_hk AS hk FROM autdbt_vault_prod.autdbtt.hub_account
    UNION ALL
    SELECT contact_hk FROM autdbt_vault_prod.autdbtt.hub_contact
    UNION ALL
    SELECT policy_hk FROM autdbt_vault_prod.autdbtt.hub_policy
) x
WHERE hk IS NULL;
